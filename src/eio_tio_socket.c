...
static int eioCreateUnixServerSocket(const char *socketPath)
{
    int sock;
    ...
    if ((status != 0) && (errno != ENOENT)) {
        eioDieWithError("socket file unlink failed\n");
    }
    ...
    const char *unixSocketPath)
{
    ...
    listenFd = eioCreateUnixServerSocket(unixSocketPath);
 ...
 *         filled in
 */
int eioTioSocketRead(int socketFd, char *msgBuff, size_t bufferSize)
{
    int cnt;
